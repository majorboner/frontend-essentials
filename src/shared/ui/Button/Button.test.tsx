...
describe('Button', () => {
  ...
  test('Параметр square применяется', async () => {
    render(<Button theme={ThemeButton.OUTLINE} square>Squared</Button>);
    expect(screen.getByText('Squared')).toHaveClass('square');
  });
  test('Размеры применяются', async () => {
    render(<Button theme={ThemeButton.OUTLINE} square size={ButtonSize.M}>SquaredSizeM</Button>);
    expect(screen.getByText('SquaredSizeM')).toHaveClass('size-m');
    render(<Button theme={ThemeButton.OUTLINE} square size={ButtonSize.L}>SquaredSizeL</Button>);
    expect(screen.getByText('SquaredSizeL')).toHaveClass('size-l');
    render(<Button theme={ThemeButton.OUTLINE} square size={ButtonSize.XL}>SquaredSizeXL</Button>);
